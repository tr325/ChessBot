...
class MoveTest(unittest.TestCase):
    ...
    def test_is_valid_board_limits(self):
        ...
        self.assertFalse(move.Move(b, q, T(-1,1,1)).is_valid())
        self.assertFalse(move.Move(b, q, T(1,-1,1)).is_valid())
        n = pc.Knight(6,6)
        self.assertTrue(move.Move(b, n, T(-2,1,1)).is_valid())
        self.assertFalse(move.Move(b, n, T(2,1,1)).is_valid())
        self.assertFalse(move.Move(b, n, T(-1,2,1)).is_valid())
    ...
    def test_apply(self):
        b = pc.Bishop(5,5)
        m = move.Move(board.Board(), b, T(-1,-1,3))
        m.is_valid()
        m.apply()
